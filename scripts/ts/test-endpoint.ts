...
async function testAtomicSwapEndpoint(): Promise<void> {
  ...
  try {
    ...
    if (result.success) {
      ...
      console.log(
        `   • Lightning Payment Hash: ${result.lightningPaymentHash}`
      );
      ...
      console.log(
        `   • Error Message: ${result.error?.message || result.message}`
      );
...
function validateSuccessResponse(response: AtomicSwapResponse): void {
  ...
  const missingFields = requiredFields.filter(
    (field) => !response[field as keyof AtomicSwapResponse]
  );
 ...
 */
function logTroubleshootingInfo(
  result: AtomicSwapResponse,
  statusCode: number
): void {
  console.log("");
  ...
  switch (result.error?.code) {
    case "MISSING_REQUIRED_FIELDS":
      ...
      console.log(
        "   • Required: amountSats, lightningDestination, tokenAddress"
      );
      break;
    ...
    case "TOKEN_NOT_FOUND":
      ...
      console.log(
        "   • Check available tokens or use a supported token address"
      );
      break;
